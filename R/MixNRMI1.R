...
MixNRMI1bis <-
  function(x, probs = c(0.025, 0.5, 0.975), Alpha = 1, Kappa = 0,
    ...
    for (j in seq(Nit)) {
      ...
      Allocs[[max(1, j - 1)]] <- idx
      if (Gama != 0) {
        # u <- gs3(u, n = n, r = r, alpha = Alpha, beta = Kappa, gama = Gama, delta = optimal_delta[r])
        # delta_U <- optimal_delta[r]
        tmp = gs3_adaptive3(ut = u, n = n, r = r, alpha = Alpha, beta = Kappa, gama = Gama, delta = delta_U, U = U, iter = j, adapt = adaptive)
        u = tmp$u_prime
        delta_U = tmp$delta
      }
      JiC <- MvInv(
