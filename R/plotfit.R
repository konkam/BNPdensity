...
plotfit_noncensored <- function(fit) {
  ...
  data.frame(xx = fit$xx, infCI = fit$qx[, 2], supCI = fit$qx[, m], y = fit$qx[, 1]) %>%
    ggplot(aes(x = xx)) +
    theme_classic() +
    geom_histogram(data = data.frame(x = fit$x), aes(x = x, y = ..density..), fill = grey(0.9), colour = "black") +
    geom_line(aes(y = y), size = 1.) +
    geom_line(aes(y = infCI), colour = "blue", linetype = "dotted") +
    geom_line(aes(y = supCI), colour = "blue", linetype = "dotted") +
    xlab("Data") +
    ylab("Density")
}
